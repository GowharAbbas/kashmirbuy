...
const ProductListing = () => {
  ...
  const buildUrl = () => {
    ...
    if (third) {
      setTitle(third);
      return `/api/product/getAllProductsByThirdSubCatName?thirdSubCat=${third}`;
    }

    if (sub) {
      setTitle(sub);
      return `/api/product/getAllProductsBySubCatName?subCat=${sub}`;
    }

    if (category) {
      setTitle(category);
      return `/api/product/getAllProductsByCatName?catName=${category}`;
    }
  ...
  const loadProducts = async () => {
    ...
    try {
      ...
      console.error("ProductListing error:", err);
      setProducts([]);
    }
  ...
  return (
    <section className="bg-white !py-6 !mt-4">
      <div className="container">
        ...
        {/* End message */}
        {!loading && products.length > 0 && (
          <p className="text-center !py-6 text-gray-600 text-[15px]">
            🎉 All products loaded
          </p>
        )}
      </div>
